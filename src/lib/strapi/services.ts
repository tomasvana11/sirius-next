...
export async function getBlogPosts() {
  const response = await strapiRequest<StrapiResponse<any[]>>("blog-posts", {
...
export async function getAboutPage() {
  const response = await strapiRequest<StrapiResponse<any>>("about-page", {
    populate: {
...
export async function getBlogs(limit?: number | "all"): Promise<BlogPost[]> {
  try {
    const params: Record<string, any> = {
      ...
      populate: ["coverImage"], // Změna: coverImage místo FeaturedImage
    };
  ...
  try {
    let response = await strapiRequest<BlogsResponse>(
      "blogs",
      {
        ...
        populate: ["coverImage"], // Změna: coverImage
      }
    );
