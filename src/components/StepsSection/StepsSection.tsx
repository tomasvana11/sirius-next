          ...
          {stepsData.Step.map((step, index) => (
            <div key={step.id} className="text-white">
              <div className="flex items-center justify-between mb-4 lg:mb-5">
                ...
                {index < stepsData.Step.length - 1 && (
                  <img
                    src="/icons/system/progress-arrow-gradient.svg"
                    alt=""
                    className="w-auto h-4 mr-2 opacity-100 hidden lg:block"
                  />
                )}
              </div>
    ...
    <section className="bg-gradient-to-b from-[#EC4C19] to-[#EB361C] px-4 lg:px-12 py-8 lg:py-12 rounded-xl relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-16">
          {stepsData.Step.map((step, index) => (
            <div key={step.id} className="text-white">
              {/* Horní řádek: ikonka + arrow */}
              <div className="flex items-center justify-between mb-4 lg:mb-5">
                <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center">
                  <Icon
                    name={step.Icon?.Icon || null}
                    size="L"
                    variant="orange" // PŘIDÁNO - oranžová varianta
                    className="w-6 h-6"
                  />
                </div>

                {/* Arrow – jen pokud to není poslední krok */}
                {index < stepsData.Step.length - 1 && (
                  <img
                    src="/icons/system/progress-arrow-gradient.svg"
                    alt=""
